...
<!--
	...
	function ajax_getZipcodeList() {
		...
		$.ajax({ //for landNum
			url : './ajax/ajax_getLandnumZipcodeList.jsp?keyword=' + encodeURI(keyword),
			dataType : 'json',
			type : 'get',
			data : {},
			success : function(result) {
		...
		$.ajax({ //for roadName
			url : './ajax/ajax_getRoadnameZipcodeList.jsp?keyword=' + encodeURI(keyword),
			dataType : 'json',
			type : 'get',
			data : {},
			success : function(result) {
	...
	<div id="layout">
		...
		<article id="mainArticle">
			<form name="frm" onsubmit="return validationCheck();"
				action="./insertUser.jsp" method="post">
				<div id="div_title">
					회원가입
